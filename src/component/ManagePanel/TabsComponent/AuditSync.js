...
import {Select,DatePicker,Form,Button,Icon,Table,Checkbox,Dropdown,Menu,message,Modal} from 'antd';
...
class  AuditSync extends Component {
  ...
  handelRequestDownload=()=>{
    ...
    if (selectedDown.length<=0) {
        message.warning("请勾选需要下载申请的数据");
    }else{
      let text="selectedDown="+selectedDown+"&bizCode="+config.bizCode;
  ...
  showApplyConfirm=()=>{
   ...
   confirm({
     ...
     cancelText: '否',
     onOk() {
       that.handleDownloadDetail();
     },
     onCancel() {
       that.setState({selectedDown:[]},()=>{that.fetch()});
     },
   });
 }
 ...
 renderDownTxt =(text,record) =>{
   const {config}=this.props;
   const {selectedDown}=this.state;
  //  if (abletodown===true||text==='able'||text==='temp') {
     let value=config.dataScope+"|"+record.CITYCODE+"|"+record.DATATYPE+"|"+record.DIFFTYPE
     +"|"+record.AUDITTIME+"|"+record.OBTAINDATATIME+"|"+record.DID+"|"+record.DIFFCODE;
     value=encodeURI(value);
     return <Row>
               <Col span={12}>
                 <Checkbox onChange={this.onCheckChange} value={value+"%23downXls"} checked={selectedDown.indexOf(value+"%23downXls")===-1?false:true}>XLS<Icon type="download" /></Checkbox>
               </Col>
               <Col span={12}>
                  <Checkbox onChange={this.onCheckChange} value={value+"%23downTxt"} checked={selectedDown.indexOf(value+"%23downTxt")===-1?false:true}>TXT<Icon type="download" /></Checkbox>
               </Col>
             </Row>
  //  }
 }
  ...
  // 设置 打开界面
    ...
    handleDownloadDetail=()=>{
      ...
      if (selectedDown.length<=0) {
        ...
        let selectedDiffs=selectedDown;
  ...
  render(){
    ...
    return(
      <div>
        ...
        handelRequestDownload={this.handelRequestDownload} permission={this.props.permission} />
        ...
        <SynCheck ref={(ref) => this.syncheck = ref}  refresh={this.fetch}  id={id}/>
...
class DiffMapSearch  extends Component {
  render() {
    ...
    let havePerm=(permission.data.indexOf('downLoad')===-1?'none':'inline');
    let applyPerm=(permission.data.indexOf('downLoad')===-1?'inline':'none');
       ...
       <Col span={2}>
        ...
        <Col span={2} >
         <FormItem {...formItemLayout} label="">
           {getFieldDecorator("handleCpyChange")(
             <Select style={{ width: 120 }} placeholder="牵头处理单位" allowClear={true}  onChange={this.handleChange}>
                 {handleCompanys.map(d =><Option key={d.dicName} value={d.dicName}>{d.dicName}</Option>)}
             </Select>
           )}
         </FormItem>
         </Col>
         ...
         <Col span={9} style={{ textAlign: 'center' }} >
          ...
          <Button style={{ marginLeft: 8 , display:havePerm }} onClick={this.props.handleDownloadDetail}> 下载差异</Button>
         </Col>
       </Row>
     </Form>
   );
